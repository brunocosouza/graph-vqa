...
def _load_visualgenome(dataroot, name, img_id2val, label2ans, adaptive=True):
    ...
    if os.path.isfile(cache_path):
        ...
        print("%s not a file, create new " % cache_path)
        entries = []
...
def _create_flickr_entry(img, sentence, entity_indices, target_indices, entity_ids, entity_types):
    type_map = {'people': 0, 'clothing': 1, 'bodyparts': 2, 'animals': 3, 'vehicles': 4, 'instruments': 5, 'scene': 6,
                'other': 7}
    MAX_TYPE_NUM = 3
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, dictionary, dataroot='data', adaptive=False):
        ...
        assert name in ['train', 'val', 'valval', 'valtest', 'test-dev2015', 'test2015']
...
def load_tsv(tsv_file: str):
    split = tsv_file.split('/')[-1].split('.')[0]
...
class PVQAFeatureDataset(Dataset):
    def __init__(self, name, dictionary, dataroot='data/pvqa', adaptive=False, img_v=''):
        ...
        self.image_data = load_tsv(tsv_file)
...
class VisualGenomeFeatureDataset(Dataset):
    def __init__(self, name, features, spatials, dictionary, dataroot='data', adaptive=False, pos_boxes=None):
        ...
        self.img_id2idx = pickle.load(
            open(os.path.join(dataroot, '%s%s_img_id2idx.pkl' % (name, '' if self.adaptive else '36')), 'rb'))
